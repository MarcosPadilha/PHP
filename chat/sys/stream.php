...
$expira = date('Y-m-d H:i:s', strtotime('+1min'));
...
if($_GET['timestamp'] == 0){
	$verifica = BD::conn()->prepare("SELECT * FROM mensagens WHERE lido = 0 ORDER BY id DESC ");
		...
		$verifica = BD::conn()->prepare("SELECT * FROM mensagens WHERE time >= $timestamp ");
	...
	if($resultados <= 0){
		while ($resultados <= 0) {
				if($resultados <= 0){
					...
					//descansar o script por 1 segundo
					sleep(1);
						$verifica = BD::conn()->prepare("SELECT * FROM mensagens WHERE lido = 0 ORDER BY id DESC ");
						$verifica->execute();
						$resultados = $verifica->rowCount();
 ...
 if($resultados>=1){
 	$emotions = array(':',':@',':8', ':D',':3',':(',';)');
		$imgs = array('<img src="emotions/nice.png" width="14"/>',
