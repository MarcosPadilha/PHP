jQuery(function(){
	...
	function add_janela(id, nome, status){//cria a janela da conversa clicada
...
janela+='<div class="header_window"> <a href="#" class="close">X</a><span class="name">'+nome+'</span><span id="'+id+'"  class="'+status+'" ></span></div>';
...
function retorna_historico(id_conversa){//envia ajax para historico.php e retorna mensagens da conversa
jQuery.ajax({
type:'POST',
...
success: function(retorno){
	jQuery.each(retorno, function(i,msg){
		var teste= msg.mensagem;
		//alert(teste);
		console.log(msg.mensagem);
		if(jQuery('#janela_'+msg.janela_de).length >0){
			if(userOnline == msg.id_de){
				jQuery('#janela_'+ msg.janela_de + ' .mensagens ul').append('<li id="'+msg.id_de+'" class="eu"><p> '+msg.mensagem+'</p> </li>');
				
			}else{
					jQuery('#janela_'+ msg.janela_de+ ' .mensagens ul').append('<li id="'+msg.id_para+'"><div class="imgSmall"><img src="fotos/'+msg.fotoUser+'"/></div><p>'+msg.mensagem+'</p></li>');
 ...
 retorna_historico(idJanela);
	...
	jQuery('body').on('click','.close', function(){
		...
		for (var i=1; i<= restamAfrente; i++){
			jQuery('.window:eq('+(indice + i)+')').animate({left:"-=275"},400);
	...
	function verifica(timestamp, lastid, user){
		...
		jQuery.ajax({
							...
							if(!in_array(msg.janela_de, clicou)){
								if(jQuery(' .mensagens ul li#'+msg.id).length == 0 && msg.janela_de > 0){
									if(userOnline == msg.id_de){
										...
										jQuery('#janela_'+msg.janela_de+' .mensagens ul').append('<li id="'+msg.id+'"><div class="imgSmall"><img src="fotos/'+msg.fotoUser+'" border="0"/></div></li>');
			...
			error: function(retorno){
				clearInterval(t);
				t=setTimeout(function(){
					verifica(rertorno.timestamp,retorno.lastid,userOnline);
